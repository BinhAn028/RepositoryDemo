...
export function PushNotificationManager() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        await OneSignal.login('huyi_abi2');
  ...
  async function postMessageToServiceWorker() {
    ...
    if (registration.active) {
      ...
      registration.active.postMessage({ type: 'PING', message: 'Web Hello Service Worker!' });
    }
  }
    ...
    <div>
      ...
      {subscription ? (
        <>
          ...
          <button onClick={postMessageToServiceWorker}>Send Service Worker</button>
        </>
      ) : (
        <>
